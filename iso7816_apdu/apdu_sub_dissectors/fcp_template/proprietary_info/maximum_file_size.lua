...
local p = Proto.new("iso7816.apdu.proprietary_info.maximum_file_size", "- Maximum File Size")
local pf = {
    section = ProtoField.string(p.name .. ".section", "Section"),
    size = ProtoField.string(p.name .. ".size", "Maximum File Size"),
}
p.fields = pf
...
function p.dissector(buffer, pinfo, tree)
    ...
    local subtree = tree:add(p, buffer(0, buffer:len()), string.format('- Maximum File Size: %s bytes', size))

    -- see (TS 102 221) 11.1.1.4.6.7 - Maximum file size
